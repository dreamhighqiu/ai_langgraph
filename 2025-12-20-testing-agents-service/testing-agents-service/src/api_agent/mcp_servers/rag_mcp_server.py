...
class ChunkData(BaseModel):
    ...
    reference_id: str = Field(default="", description="引用 ID")
# pylint: disable  MC80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2Y2tONlp3PT06OTc1NzU2NWU=
...
# pylint: disable  MS80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2Y2tONlp3PT06OTc1NzU2NWU=
...
# pylint: disable  Mi80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2Y2tONlp3PT06OTc1NzU2NWU=
...
async def rag_query_data(
    ...
    🎯 主要用途：
    - 检索API接口的详细信息（URL、参数、认证方式、请求/响应格式等）
    - 查找API接口的使用示例和测试数据
    - 获取接口的历史测试记录和基准值
    - 了解接口的依赖关系和相关接口

    📋 使用场景：
    1. 当用户提到具体的API接口名称时（如"首页接口"、"登录接口"、"用户信息接口"）
    2. 需要了解接口的技术细节以生成测试计划或测试脚本时
    3. 需要查找接口的历史测试数据和性能基准时

    🔧 查询模式说明：
    - mix (推荐): 结合知识图谱和向量检索，获取最全面的接口信息
    - local: 获取接口及其直接相关的实体和关系
    - naive: 仅使用向量相似性搜索文档内容
    - global: 探索知识图谱中的全局关系模式
    - hybrid: 结合本地和全局检索策略

    📝 参数说明：
        query: 接口名称或功能描述（如"获取首页信息接口"、"用户登录API"）
        mode: 检索模式，推荐使用 "mix" 获取最全面的信息
    ...
    💡 使用示例：
        检索首页接口信息：
            rag_query_data(query="获取首页信息接口的详细信息，包括URL、参数、认证方式", mode="mix")

        查找登录接口的测试数据：
            rag_query_data(query="用户登录接口的测试用例和历史测试数据", mode="mix", top_k=15)

        了解接口的依赖关系：
            rag_query_data(query="订单创建接口的依赖接口和前置条件", mode="local")
    """
    ...
    try:
        ...
        if response.data:
            ...
            if data.entities:
                ...
                output += "（无实体数据）\n"
# pylint: disable  My80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2Y2tONlp3PT06OTc1NzU2NWU=
