...
# RAG 检索示例（必须传递 project_id）
result = await rag_query_tool(
    query="用户登录接口的详细信息",
    mode="mix",
    project_id={project_id}  # 必须使用上下文中的 project_id
)
if result["success"]:
...
**使用场景**（在以下情况必须使用）：
- **用户明确要求使用 RAG 检索**（包括在提示词中提到"使用 RAG 检索：是"、"启用 RAG"、"使用知识库"等）
- 需要了解具体的 API 接口技术细节、参数格式等
- 查找历史测试数据和性能基准
- **重要**：如果用户的提示词中包含"使用 RAG 检索：是"或类似的明确指示，**必须立即调用此工具**，不要询问用户
