"""
UI自动化测试执行记录视图对象(VO - View Object)
"""
...
class UIExecutionModel(BaseModel):
    """UI执行记录模型"""
    ...
    browser: Optional[str] = Field('chromium', description='执行浏览器')
    headless: Optional[str] = Field('1', description='是否无头模式（0否 1是）')
...
class UIExecutionPageQueryModel(BaseModel):
    """UI执行记录分页查询模型"""
...
class UIExecutionDetailModel(BaseModel):
    """UI执行详情模型"""
    model_config = ConfigDict(from_attributes=True)
    ...
    browser: str = Field(..., description='执行浏览器')
    headless: str = Field(..., description='是否无头模式')
    start_time: Optional[datetime] = Field(None, description='开始时间')
    end_time: Optional[datetime] = Field(None, description='结束时间')
...
class CancelUIExecutionModel(BaseModel):
    """取消UI执行模型"""
    execution_id: int = Field(..., description='执行ID')
