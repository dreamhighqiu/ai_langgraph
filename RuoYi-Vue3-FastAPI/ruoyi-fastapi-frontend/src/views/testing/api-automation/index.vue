<template>
  <div class="api-automation-workspace">
    <el-page-header @back="$router.back()" content="API自动化测试工作台">
      <template #extra>
    ...
    <el-card class="mt-4" v-if="currentProject">
      <template #header>
        <div class="card-header">
          ...
          <el-tag type="warning">API自动化</el-tag>
        </div>
      </template>
    ...
    <!-- 项目选择对话框 -->
    <el-dialog title="选择API自动化项目" v-model="showProjectSelector" width="600px">
      <el-table
        :data="apiProjects"
...
<script setup name="APIAutomation">
...
function getAPIProjects() {
  listProject({
    ...
    apiProjects.value = response.data.rows
    if (apiProjects.value.length > 0 && !currentProject.value) {
      currentProject.value = apiProjects.value[0]
      loadStatistics()
    }
  })
}
...
function handleGenerateScript(scriptData) {
  ...
  if (scriptData && scriptData.script_id) {
    ...
    proxy.$modal.msgSuccess(`API测试脚本已生成，ID: ${scriptData.script_id}`)
  }
}
...
function handleExecuteScript(executionData) {
  ...
  if (executionData && executionData.execution_id) {
    ...
    proxy.$modal.msgSuccess(`API测试开始执行，ID: ${executionData.execution_id}`)
  }
}
