...
// NOTE  MC80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2Wm5SQldnPT06NjExZmUyOTA=
...
// FIXME  Mi80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2Wm5SQldnPT06NjExZmUyOTA=
...
export const FolderTree = React.forwardRef<FolderTreeRef, FolderTreeProps>(function FolderTree({
  ...
  const loadChildren = async (folderId: string) => {
    try {
      ...
      if (response.success && response.data) {
        setFolders((prev) =>
          updateFolderChildren(prev, folderId, response.data)
        );
      }
    } catch (error) {
  ...
  const reloadExpandedFolders = async (nodes: FolderTreeNode[]): Promise<FolderTreeNode[]> => {
    ...
    for (const node of nodes) {
      ...
      if (isExpanded && node.sub_folders_count > 0) {
        try {
          const response = await getFolders(projectId, node.id);
          if (response.success && response.data) {
            // 递归重新加载子文件夹
            const childNodes = response.data.map((c) => ({
              ...c,
              children: undefined,
              loading: false,
              expanded: false,
...
// FIXME  My80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2Wm5SQldnPT06NjExZmUyOTA=
