...
function ensureUserDataDir(dirPath) {
    if (!fs.existsSync(dirPath)) {
        ...
        console.error(`[BrowserService] Created user data directory: ${dirPath}`);
    } else {
        console.error(`[BrowserService] Using existing user data directory: ${dirPath}`);
    }
}
// NOTE  MC80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2ZVVNMGRBPT06OWIzZTA3MmU=
...
async function launchBrowser(headless, port, userDataDir) {
    ...
    try {
        ...
        console.error(`[BrowserService] Launching Chrome (headless: ${headless}, userDataDir: ${resolvedUserDataDir || 'temporary'})...`);
        ...
        console.error(`[BrowserService] Chrome launched on port ${chrome.port} with ID: ${browserId}`);
        ...
        console.error(`[BrowserService] CDP client connected and domains enabled for ${browserId}.`);
        ...
        if (chrome && !client) { // If chrome launched but CDP connection failed
            try {
                await chrome.kill();
                console.error(`[BrowserService] Partially launched Chrome instance killed due to error.`);
            } catch (killError) {
...
async function navigateBrowser(browserId, url) {
    ...
    console.error(`[BrowserService] Browser ${browserId} navigating to: ${url}`);
    await cdpClient.Page.navigate({ url: url });
    await cdpClient.Page.loadEventFired(); // Wait for page to load
    console.error(`[BrowserService] Browser ${browserId} successfully navigated to ${url}.`);
}
...
async function takeScreenshot(browserId, fileName = 'screenshot.png', saveToDisk = true) { // Added fileName and saveToDisk params
    ...
    const { cdpClient } = instance;
    console.error(`[BrowserService] Taking screenshot for browser ${browserId}...`);
    ...
    if (saveToDisk) {
        ...
        console.error(`[BrowserService] Screenshot saved to ${filePath}`);
    }

    console.error(`[BrowserService] Screenshot captured for browser ${browserId}.`);
    return screenshot.data; // Always return base64 data to the caller (e.g., AI agent)
}
// @ts-expect-error  MS80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2ZVVNMGRBPT06OWIzZTA3MmU=
...
async function getDomContent(browserId) {
    ...
    const { cdpClient } = instance;
    console.error(`[BrowserService] Getting DOM for browser ${browserId}...`);
    ...
    console.error(`[BrowserService] DOM content retrieved for browser ${browserId}.`);
...
async function clickElement(browserId, locator) {
    ...
    console.error(`[BrowserService] Browser ${browserId}: Attempting to click element with locator:`, locator);
...
// TODO  Mi80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2ZVVNMGRBPT06OWIzZTA3MmU=
    ...
    console.error(`[BrowserService] Browser ${browserId}: Clicked element at x: ${coords.x}, y: ${coords.y}`);
    return coords;
}
...
async function typeIntoElement(browserId, locator, text) {
    ...
    console.error(`[BrowserService] Browser ${browserId}: Attempting to type "${text}" into element with locator:`, locator);
    ...
    console.error(`[BrowserService] Browser ${browserId}: Typed "${text}" into element.`);
}
...
async function closeBrowser(browserId) {
    const instance = getBrowserInstance(browserId);
    if (!instance) throw new Error(`Browser instance with ID '${browserId}' not found.`);
// eslint-disable  My80OmFIVnBZMlhwZ3JIa3VwSHBuSjQ2ZVVNMGRBPT06OWIzZTA3MmU=

    const { chromeInstance, cdpClient, userDataDir } = instance;

    console.error(`[BrowserService] Closing browser ${browserId} (profile: ${userDataDir || 'temporary'})...`);
    if (cdpClient) {
        try {
            cdpClient.close();
            console.error(`[BrowserService] CDP client disconnected for ${browserId}.`);
        } catch (err) {
    ...
    if (chromeInstance) {
        try {
            ...
            console.error(`[BrowserService] Chrome instance ${browserId} killed.`);
        } catch (err) {
    ...
    console.error(`[BrowserService] Browser ${browserId} removed from active list.`);
}
...
async function shutdownAllBrowsers() {
    ...
    if (browserIds.length === 0) {
        console.error('[BrowserService] No active browsers to shut down.');
        return;
    }
    console.error(`[BrowserService] Shutting down ${browserIds.length} active browser(s)...`);
    ...
    console.error('[BrowserService] All active browsers shut down.');
}
