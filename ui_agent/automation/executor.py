...
def create_playwright_executor_tool() -> BaseTool:
    ...
    def run_playwright_script(
        ...
        playwright_project_dir = _find_playwright_project_dir(
            cfg.workspace_root, actual_script_path
        )
        ...
        try:
            ...
            return json.dumps(
                {
                    "success": False,
                    "error": "Playwright execution timeout (10 minutes).",
            ...
            return json.dumps(
                {
                    "success": False,
                    "error": f"Playwright not found: {cfg.playwright_binary}",
                    "script_path": script_path,
                    "run_id": run_id,
                    "run_dir": layout.run_dir_virtual,
                },
    ...
    return StructuredTool.from_function(
        ...
        description=(
            "Run Playwright script with isolated report output.\n"
            "Args:\n"
            "- script_path: virtual path (e.g. /playwright_scripts/tests/test_xxx.spec.ts)\n"
            "- browser: chromium/firefox/webkit\n"
            "- headless: optional\n"
            "- reporter: optional (default html,json)\n"
            "Returns JSON with report paths and summary."
        ),
    )


def _find_playwright_project_dir(workspace_root: str, actual_script_path: Path) -> Path:
    parts = list(actual_script_path.resolve().parts)
    if "playwright_scripts" in parts:
        idx = parts.index("playwright_scripts")
        return Path(*parts[: idx + 1])

    candidate = Path(workspace_root).resolve() / "playwright_scripts"
    if candidate.exists():
        return candidate

    raise FileNotFoundError(f"playwright_scripts directory not found (workspace_root={workspace_root})")
